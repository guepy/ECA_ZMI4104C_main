...
void MainWindow::updateLeftBlockValue(){
    ...
    ui->biasModeAxis_1->setText(QString(biasMode[biasModeTab[0]]));
    ui->biasModeAxis_1->setAlignment(Qt::AlignCenter);
    ui->biasModeAxis_2->setText(QString(biasMode[biasModeTab[1]]));
    ui->biasModeAxis_2->setAlignment(Qt::AlignCenter);
    ui->biasModeAxis_3->setText(QString(biasMode[biasModeTab[2]]));
    ui->biasModeAxis_3->setAlignment(Qt::AlignCenter);
    ui->biasModeAxis_4->setText(QString(biasMode[biasModeTab[3]]));
...
void MainWindow::selectLeftBlockValue(int index){
    ...
    for(int i=0; i<4; i++){
        if(getBiasMode(i+1,(uint32_t*)(biasModeTab+i))!= RET_SUCCESS)
            std::cerr<<"set bia mode failed"<<std::endl;
    }
}
