...
void ZygoZMI4104C::get_device_property()
{
	...
	DEBUG_STREAM << "ZygoZMI4104C::get_device_property()" << endl;
...
void ZygoZMI4104C::read_samplingFrequency(Tango::Attribute &attr)
{
	...
	if(getSamplingFrequency(attr_samplingFrequency_read) != RET_SUCCESS)
	{
		ERROR_STREAM << "read_samplingFrequency: error: " << std::endl;
	}
...
void ZygoZMI4104C::write_samplingFrequency(Tango::WAttribute &attr)
{
	...
	if(setSamplingFrequency((unsigned int) w_val) != RET_SUCCESS)
	{
		ERROR_STREAM << "write_samplingFrequency: error: " << std::endl;
	}
...
//--------------------------------------------------------
/**
 *	Function vme_system_reset TODO
 */
//--------------------------------------------------------
void ZygoZMI4104C::vme_system_reset()
{
	INFO("VME SYSTEM RESET!!!\n");
	DEBUG_STREAM << "start initSISboards() " ;
	if (initSISboards() != RET_SUCCESS)
	{
		//FATAL("Failed to initialize SIS boards\n");
		ERROR_STREAM << "fsdfdsf " << endl;
		return;
		
	}
	DEBUG_STREAM << "end initSISboards() " ;
	//Sleep(10);
	if (initZMIboards() != RET_SUCCESS) //FATAL("Failed to initialize ZMI board\n");
	DEBUG_STREAM << "end initZMIboards() " << endl;
	//ui->pushButton_11->setEnabled(false);
	//bias mode
	BIAS_MODE bias_mode;
	bias_mode = BIAS_CONSTANT_VOLT_MODE;
	if (initAxis(bias_mode) != RET_SUCCESS) //FATAL("Failed to initialize axis\n");
	DEBUG_STREAM << "end initAxis() "<< endl;
	EnableDoublePassInterferometer();	
	DEBUG_STREAM << "end EnableDoublePassInterferometer() " << endl;
	Tango::DevULong	w_val;
	w_val=getBaseAddress();
	DEBUG_STREAM << "base address is "<<w_val <<"and getBA "<<getBaseAddress( )<< endl;
}
