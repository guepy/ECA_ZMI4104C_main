...
//================================================================
//  samplingunsigned int*y                |  Tango::DevULong	Scalar
...
void ZygoZMI4104C::delete_device()
{
	...
	delete[] attr_samplingunsigned int*y_read;
...
void ZygoZMI4104C::init_device()
{
	...
	attr_samplingunsigned int*y_read = new Tango::DevULong[1];
	...
	attr_currentPrecision_read = new currentPrecisionEnum[1];
	...
	axisTab[sizeof(axisTab)/sizeof(axisTab[0])-1] = '\0';
	optPwr[4]='\0';
...
void ZygoZMI4104C::read_attr_hardware(TANGO_UNUSED(vector<long> &attr_list))
{
	...
	{
		DEBUG_STREAM << "ledsColor[" << ledi<<"]: "<<(int)ledsColor[ledi] << endl;
		DEBUG_STREAM<<"LEDSCOLOR AXIS "<< ledi+1<<" is "<< ledsColorString[(int)ledsColor[ledi]]<<endl;
	}
...
//--------------------------------------------------------
/**
 *	Read attribute samplingunsigned int*y related method
 *	Description: 
 *
 ...
 */
//--------------------------------------------------------
void ZygoZMI4104C::read_samplingunsigned int*y(Tango::Attribute &attr)
{
	DEBUG_STREAM << "ZygoZMI4104C::read_samplingunsigned int*y(Tango::Attribute &attr) entering... " << endl;
	/*----- PROTECTED REGION ID(ZygoZMI4104C::read_samplingunsigned int*y) ENABLED START -----*/
	//	Set the attribute value
	getSamplingunsigned int*y((uint32_t*)attr_samplingunsigned int*y_read);
	attr.set_value(attr_samplingunsigned int*y_read);
	
	/*----- PROTECTED REGION END -----*/	//	ZygoZMI4104C::read_samplingunsigned int*y
}
//--------------------------------------------------------
/**
 *	Write attribute samplingunsigned int*y related method
 *	Description: 
 *
 *	Data type:	Tango::DevULong
 *	Attr type:	Scalar
 */
//--------------------------------------------------------
void ZygoZMI4104C::write_samplingunsigned int*y(Tango::WAttribute &attr)
{
	DEBUG_STREAM << "ZygoZMI4104C::write_samplingunsigned int*y(Tango::WAttribute &attr) entering... " << endl;
	//	Retrieve write value
	Tango::DevULong	w_val;
	attr.get_write_value(w_val);
	/*----- PROTECTED REGION ID(ZygoZMI4104C::write_samplingunsigned int*y) ENABLED START -----*/
	
	setSamplingunsigned int*y((unsigned int)w_val);
	
	/*----- PROTECTED REGION END -----*/	//	ZygoZMI4104C::write_samplingunsigned int*y
}
//--------------------------------------------------------
/**
...
void ZygoZMI4104C::read_currentPrecision(Tango::Attribute &attr)
{
	...
	//	Set the attribute value
	*attr_currentPrecision_read = (getPrecision()? currentPrecisionEnum::_REG37 : currentPrecisionEnum::_REG32);
...
void ZygoZMI4104C::write_interferometerConfiguration(Tango::WAttribute &attr)
{
	...
	//len = sizeof(interferometerConfigurationString)/sizeof(interferometerConfigurationString[0]);
	
	for(int i=0; i<len_wval;i++){
			w_val[i] = (char)(toupper(w_val[i]));
		}
		std::cerr<<"w_val in interfero is " << w_val << std::endl;
	try
	{
...
void ZygoZMI4104C::write_continuousSamplingAxes(Tango::WAttribute &attr)
{
	...
	int len = ((std::string)w_val).length();
	char tempStr[100];
	myStr = "";
	axisCtr = 0;
	*attr_continuousSamplingAxes_read="";
	std::cerr << "w_val is " << w_val<<endl;
	try
	{
	for(int i=0;i<len+1;i++){
	std::cerr << "w_val[" << i <<"] is "<< w_val[i]<<endl;
		if(w_val[i]=='\0' || w_val[i]==';')
			break;
		if(w_val[i]==',')
			continue;
		
		if((w_val[i]<31 || w_val[i]>34) )//value is not between 1 and 4
		{
			std::cerr << "Unknow axis value: " << w_val[i] <<std::endl;
			throw std::invalid_argument("Unknow axis value"); 
			continue;
		}
		else
		{
			...
			axisTab[axisCtr]=(uint32_t)atoi(&w_val[i]);
			sprintf(tempStr, "AXIS%u | ",axisTab[axisCtr]);
	...
	{
		for(int i=0;i<axisCtr;i++)
		{
			std::cerr<< "selected axis are : " << axisTab[i] <<std::endl;
		}
		*attr_continuousSamplingAxes_read = (Tango::DevString)myStr.c_str();
 ...
 *	Description: Start the continuous acquisition based on sampling unsigned int*y and continuousScanSize board properties. 
 *               In the Auto mode, the user should provide sampling unsigned int*y as well as the number of sample to acquire (samplingunsigned int*y and samplingSize). The acquisition time will be determined by the board.
 *               In the manual mode, the user only provide the unsigned int*y. the size and the integration time will depend on the time at which the user press the  StopAcquisition button.
 *
...
Tango::DevShort ZygoZMI4104C::start_continuous_acquisition(Tango::DevBoolean argin)
{
    ...
    if((*attr_continuousSamplingSize_read)>NBR_RAM_PAGES*128 && axisCtr>2){
    ...
    //*/
    if (!(base_A24D32_ptr = (unsigned int*)calloc((UINT)( sizeof (UINT)*((*attr_continuousSamplingSize_read)*1.5*axisCtr)), sizeof(unsigned int)))){
	...
	if(axisCtr>1){
		if (!(base_A24D32_FR_ptr = (unsigned int*)calloc((UINT)( sizeof (UINT)*(*attr_continuousSamplingSize_read)*1.5*axisCtr), sizeof(unsigned int)))){
			//dev_mutex.unlock();
			//Ltimer->stop();
			zygoLogWarn("can not allocate memory on the host machine");
			//emit flyscanErrorCode(-105);
	...
	try
	{
		if (configureFlyscan(  axisCtr, flyscanFreqValue, 1) != RET_SUCCESS){
			throw "configure flyscan function failed ";
		}
    }
	}
	...
	if (getFlyscanData(  base_A24D32_FR_ptr, base_A24D32_ptr, &axisCtr,*attr_continuousSamplingSize_read) != RET_SUCCESS)
    {
        //dev_mutex.unlock();
        //Ltimer->stop();
    ...
    if (processRamData(axisCtr, base_A24D32_FR_ptr, base_A24D32_ptr, ramDataSize, flyscanPath,(double*)meanVal, (double*)stdDevVal) != RET_SUCCESS)
    {
        //Ltimer->stop();
